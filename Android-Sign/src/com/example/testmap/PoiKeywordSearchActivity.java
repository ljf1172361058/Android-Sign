...
public class PoiKeywordSearchActivity extends FragmentActivity implements
	...
	private AMap aMap;
	private AutoCompleteTextView searchText;// 输入搜索关键�?
	private String keyWord = "";// 要输入的poi搜索关键�?
	private ProgressDialog progDialog = null;// 搜索时进度条
	private EditText editCity;// 要输入的城市名字或�?�城市区�?
	private PoiResult poiResult; // poi返回的结�?
	private int currentPage = 0;// 当前页面，从0�?始计�?
	...
	private void setUpMap() {
		...
		searchText.addTextChangedListener(this);// 添加文本输入框监听事�?
	...
	public void nextButton() {
		if (query != null && poiSearch != null && poiResult != null) {
			if (poiResult.getPageCount() - 1 > currentPage) {
				currentPage++;
				query.setPageNum(currentPage);// 设置查后�?�?
	...
	protected void doSearchQuery() {
		...
		query.setPageSize(10);// 设置每页�?多返回多少条poiitem
		query.setPageNum(currentPage);// 设置查第�?�?
	...
	/**
	 * 调起高德地图导航功能，如果没安装高德地图，会进入异常，可以在异常中处理，调起高德地图app的下载页�?
	 */
	...
	public void onPoiSearched(PoiResult result, int rCode) {
		dissmissProgressDialog();// 隐藏对话�?
		if (rCode == 1000) {
			if (result != null && result.getQuery() != null) {// 搜索poi的结�?
				if (result.getQuery().equals(query)) {// 是否是同�?�?
					poiResult = result;
					// 取得搜索到的poiitems有多少页
					List<PoiItem> poiItems = poiResult.getPois();// 取得第一页的poiitem数据，页数从数字0�?�?
					List<SuggestionCity> suggestionCities = poiResult
							.getSearchSuggestionCitys();// 当搜索不到poiitem数据时，会返回含有搜索关键字的城市信�?
